...
import static com.bitcoinj.Utils.checkArgument;
    ...
    /**
     * A deterministic key is a node. As per
    ...
    {
        private static <T> T checkNotNull(T reference) ///<T extends Object>
        {
        ...
        public DeterministicKey(List<ChildNumber> childNumberPath,
                                ...
                                BigInteger priv, ///@Nullable
                                DeterministicKey parent) { ///@Nullable
        ...
        public DeterministicKey(List<ChildNumber> childNumberPath,
            ...
            this.encryptedPrivateKey = priv; ///checkNotNull(priv);
            this.keyCrypter = crypter; ///checkNotNull(crypter);
        }*/
        ...
        */
        ...
        public byte[] getIdentifier() {
            return sha256hash160(getPubKey());
        }

        /** Returns the first 32 bits of the result of {@link #getIdentifier()}. */
        public int getFingerprint() {
            ////TO DO why is this different than armory's fingerprint? BIP 32: "The first 32 bits of the identifier are called the fingerprint."
...
*/
       /* @Deprecated
        ...
        private byte[] serialize(NetworkParameters params, boolean pub, Script.ScriptType outputScriptType)
        {
            ...
            else throw new IllegalStateException(); ///outputScriptType.toString()
        ...
        public static DeterministicKey deserializeB58(NetworkParameters params, String base58) {
            return deserializeB58(params, base58, null);
        }
         ...
         */
        public static DeterministicKey deserializeB58(NetworkParameters params, String base58,  DeterministicKey parent) {//@Nullable
         ...
         * @param parent The parent node in the given key's deterministic hierarchy.
         */
        public static DeterministicKey deserialize(NetworkParameters params, byte[] serializedKey, DeterministicKey parent) { //@Nullable
            ...
            if (buffer.hasRemaining()) throw new IllegalArgumentException("Found unexpected data in key");

            if (pub) return new DeterministicKey(path, chainCode, new LazyECPoint(ECKey.CURVE.getCurve(), data), parent, depth, parentFingerprint);
            else return new DeterministicKey(path, chainCode, new BigInteger(1, data), parent, depth, parentFingerprint);
        }
...
*/
        private static ArrayList<ChildNumber> buildChildNumberList (String path)
        {
            // path regular expresion must match [mM](/[0-9]+['H]?)
            ArrayList<ChildNumber> list = new ArrayList<ChildNumber>();

            // tokenize path without first letter m
            String[] tokens = path.substring(1).split("/");

            // Fill ChildNumber list
            for (String token : tokens)
            {
                if (token.length() != 0)
                {
                    boolean hardened = (token.endsWith("'") | (token.endsWith("H")));
                    if (hardened) token = token.substring(0, token.length() - 1);

                    list.add(new ChildNumber(Integer.parseInt(token), hardened));
                }
            }
            return list;
        }

        public static DeterministicKey computeDerivation(DeterministicKey key, String path)
        {
            ArrayList<ChildNumber> list = buildChildNumberList(path);

            if (list.isEmpty()) return key;
            else {
                DeterministicHierarchy dh = new DeterministicHierarchy(key);
