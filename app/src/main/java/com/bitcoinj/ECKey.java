...
*/
        ///int bits = FixedPointUtil.getCombSize(CustomNamedCurves.getByName("secp256k1"));
        ///int minWidth = bits > 250 ? 6 : 5;

        // Tell Bouncy Castle to precompute data that's needed during secp256k1 calculations.
        FixedPointUtil.precompute(CURVE_PARAMS.getG(),5);
    ...
    public ECKey(SecureRandom secureRandom) {
        ...
        creationTimeSeconds = System.currentTimeMillis()/1000; ///Utils.currentTimeSeconds();
    }

    protected ECKey( BigInteger priv, ECPoint pub, boolean compressed) {//@Nullable
        this(priv, getPointWithCompression(checkNotNull(pub), compressed));
    }

    protected ECKey( BigInteger priv, LazyECPoint pub) {//@Nullable
        if (priv != null) {
            checkArgument(priv.bitLength() <= 32 * 8, "private key exceeds 32 bytes");
     ...
     */
    @Deprecated
    public ECKey( byte[] privKeyBytes,  byte[] pubKey) {//@Nullable
     ...
     */
    @Deprecated
    public ECKey( BigInteger privKey, byte[] pubKey, boolean compressed) {//@Nullable
     ...
     */
    @Deprecated
    private ECKey( BigInteger privKey,  byte[] pubKey) {//@Nullable
    ...
    public byte[] getPubKeyHash() {
        if (pubKeyHash == null)
            pubKeyHash = sha256hash160(this.pub.getEncoded());
        return pubKeyHash;
    }
...
*/
     ...
     */
    public ECKey maybeDecrypt( KeyParameter aesKey) throws KeyCrypterException {//@Nullable
...
/*    public String toStringWithPrivate(@Nullable KeyParameter aesKey, NetworkParameters params) {
    ...
    public String getPrivateKeyAsHex() {
        return Utils.HEXencode(getPrivKeyBytes());
    }

    public String getPublicKeyAsHex() {
        return Utils.HEXencode(pub.getEncoded());
    }
...
/*    public void formatKeyWithAddress(boolean includePrivateKeys, @Nullable KeyParameter aesKey, StringBuilder builder,
        ...
        builder.append(Utils.HEXencode(getPubKeyHash()));
        if (creationTimeSeconds > 0)
